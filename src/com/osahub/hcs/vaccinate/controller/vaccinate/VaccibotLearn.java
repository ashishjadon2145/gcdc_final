...
public class VaccibotLearn extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(req.getParameter("action").trim().equalsIgnoreCase("addVocab")){
			...
			// 	calculate and updating prior probabilities for classifier
					...
					resp.sendRedirect("adminPanel?status=103#vacciBot");
					
				}catch(Exception d){
					resp.sendRedirect("adminPanel?status=104#vacciBot");
				...
				resp.sendRedirect("adminPanel?status=105#vacciBot");
			...
			try{
				...
				ofy().clear();
				resp.sendRedirect("adminPanel?status=106#vacciBot");
				//resp.getWriter().println("everything completed successfully");
			} catch(Exception d){
				resp.sendRedirect("adminPanel?status=107#vacciBot");
				//resp.getWriter().println("error creating repository/vocabulary or updating prior probability");	
				//resp.getWriter().println("\nMessage  : "+d.toString());
			}
		}
